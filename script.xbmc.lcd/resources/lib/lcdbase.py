...
def StringToLCDCharSet(strText):
  #0 = HD44780, 1=KS0073
  iLCDContr = 0
  ...
  0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x5b, 0xa4, 0x5d, 0x5e, 0x5f, # 96
  0x60, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, # 112
  0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x7b, 0x7c, 0x7d, 0xb0, 0x20, # 128
  0x20, 0x20, 0x2c, 0x20, 0x22, 0x20, 0x20, 0x20, 0x5e, 0x20, 0x53, 0x3c, 0x20, 0x20, 0x5a, 0x20, # 144
  0x20, 0x27, 0x27, 0x22, 0x22, 0xa5, 0xb0, 0xb0, 0xb0, 0x20, 0x73, 0x3e, 0x20, 0x20, 0x7a, 0x59, # 160
  ...
  cLCD=""
  #strings are immutable - so we have to fuck here for changing the chars
  a1 = array("c",strText)
  iSize = len(a1)
  for i in range(1,iSize):
    a1[i] = chr(LCD[iLCDContr][int(ord(strText[i]))])
  return a1.tostring()
'''
...
#  @abstractmethod
  ...
  def GetProgressBar(self, tCurrent, tTotal):
    cLCDsmallBlocks = 0xb0 #this char (0xAC-0xAF) will be translated in LCD.cpp to the smallBlock
    cLCDbigBlock = 0xab    #this char will be translated in LCD.cpp to the right bigBlock
    iBigBlock = 5          # a big block is a combination of 5 small blocks
    iColumns = int(self.GetColumns()) - 2

    if iColumns > 0:
      dBlockSize = float(float(tTotal) * 0.99 / float(iColumns) / float(iBigBlock))  # mult with 0.99 to show the last bar
      strProgressBar = "["

      for i in range(1, iColumns+1):
        #set full blocks       
        if tCurrent >= float(i * iBigBlock * float(dBlockSize)):
          strProgressBar += chr(cLCDbigBlock)
        # set a part of a block at the end, when needed
        elif tCurrent > float((i - 1) * iBigBlock * float(dBlockSize) + float(dBlockSize)):
          tmpTest = float(tCurrent - (int(tCurrent / (iBigBlock * float(dBlockSize))) * iBigBlock * float(dBlockSize) ))
          tmpTest = float(tmpTest / float(dBlockSize))
          if tmpTest >= iBigBlock:
            tmpTest = iBigBlock - 1
          strProgressBar += chr(cLCDsmallBlocks - int(tmpTest))
